...
class Bvb_Grid_Deploy_Table extends Bvb_Grid implements Bvb_Grid_Deploy_DeployInterface {
                ...
                if ($mode == 'edit') {
                    ...
                    for ($i = 1; $i <= count($conditions); $i++) {
                        ...
                        if (($r === false || (is_array($r) && count($r)==0) ) && count($conditions) == 1) {
                            ...
                            $this->_gridSession->message = $this->__('Registro não encontrado');
    ...
    protected function _processForm()
    {
        ...
        if ($this->getRequest()->isPost() && $this->getRequest()->getPost('zfg_form_edit' . $this->getGridId()) == 1) {
            ...
            if ($this->_form->isValid($_POST)) {
                    ...
                    try {
                        foreach ($this->_form->getSubForms() as $key => $value) {
                            ...
                            if (count($post[$key]) == 0) {
                            	...
                            	throw new Bvb_Grid_Exception($this->__('Sem valores para inserir'));
                            }
                        ...
                        $this->_gridSession->message = $this->__('Registro salvo');
                        ...
                        $this->_gridSession->message = $this->__('Erro ao salvar o registro: ') . $e->getMessage();
                    ...
                    try {
                        ...
                        foreach ($this->_form->getSubForms() as $key => $value) {
                            ...
                            if (count($post[$key]) == 0) {
                            	...
                            	throw new Bvb_Grid_Exception($this->__('Sem valores para atualizar'));
                            }
                        ...
                        $this->_gridSession->message = $this->__('Erro ao atualizar o registro: ') . $e->getMessage();
                        $this->_gridSession->formSuccess = 0;
                        $this->_gridSession->formPost = 1;
                        $this->_gridSession->_noForm = 0;
                        $this->_gridSession->correct = 0;
                ...
                $this->_gridSession->message = $this->__('Erro na validação');
                $this->_gridSession->messageOk = false;
                $this->_gridSession->formSuccess = 0;
                $this->_gridSession->formPost = 1;
                $this->_gridSession->_noForm = 0;
        ...
        if ($this->getParam('postMassIds') && $this->getParam('zfmassremove') == 1) {
            ...
            foreach ($ids as $value) {
                ...
                try {
                    ...
                    $this->_gridSession->message = $this->__('Erro ao excluir o registro: ') . $e->getMessage();
                }
            }
            ...
            $this->_gridSession->message = $this->__('Registro excluído');
            $this->_gridSession->correct = 1;
    ...
    protected function _buildHeader()
    {
        ...
        if (count($this->_filters) > 0) {
            ...
            if ($this->getParam('order') && !$this->getParam('noOrder') && count($this->_filtersValues) > 0) {
                ...
                */
				$final1 =	'<a class="cancelar" href="' . $url  . '" alt="Remover filtro" title="Remover filtro" >Remover filtro</a>' .
							'<a class="cancelar" href="' . $url2 . '" alt="Remover ordenação" title="Remover ordenação" >Remover ordenação</a>' .
							'<a class="cancelar" href="' . $url3 . '" alt="Remover filtro e ordenação" title="Remover filtro e ordenação" >Remover filtro e ordenação</a>';
                ...
                */
				$final1 = '<a class="cancelar" href="' . $url2 . '" alt="Remover ordenação" title="Remover ordenação" >Remover ordenação</a>';
            ...
            if (($this->getInfo('noFilters') != 1 || $this->_allowAdd == 1)
                ...
                if (strlen($final1) > 5 || $this->getUseKeyEventsOnFilters() == false || $addButton) {
                    if ($this->getUseKeyEventsOnFilters() === false && $this->getInfo('noFilters') != 1) {
                        ...
                        */
						$final1 .= '<a class="pesquisar" href="#" onclick="_' . $this->getGridId() . 'gridChangeFilters(1)" alt="Aplicar filtro" title="Aplicar filtro">Aplicar filtro</a>';
    ...
    protected function _buildFiltersTable($filters)
    {
        ...
        foreach ($filters as $filter) {
            ...
            if ($filter['type'] == 'extraField' && $filter['position'] == 'left') {
                ...
                /*
                 * versão 2
            	$caixaDeVerificacao =	PHP_EOL .
            							"<input type='checkbox' class='selecionar' title='Marcar / Desmarcar todos' style='float: left;'>" .
            							"<a class='inverter' style=' float: left; margin-left: 3px;'>" . $images['inverter'] . "</a> " .
            							"<span class='qtde' style=' float: left; heigth: 36px; margin-left:3px;'>[0]</span> " .
            							"<input type='hidden' class='registros'>"
            							;
            	* versão 3
            	$caixaDeVerificacao =	PHP_EOL .
            							"<a class='tudo' >" . $images['tudo'] . "</a> " .
            							"<a class='nada' >" . $images['nada'] . "</a> " .
            							"<a class='inverter' >" . $images['inverter'] . "</a> " .
            							"<span class='qtde' title='Quantidade de itens marcados' style='width: 16px; height: 16px; text-align: center; background: url(" . $images['fundo'] . ") no-repeat; float: left; margin-top: 1px; display: block; ' >0</span> " .
            							//"<span class='qtde' title='Quantidade de itens marcados' style='width: 16px; height: 16px; text-align: center; background-color: silver; no-repeat; float: left; margin-top: 1px; display: block; ' >0</span> " .
            							"<input type='hidden' class='registros'>"
            							;
            	*/
            	$caixaDeVerificacao =	PHP_EOL .
            							"<input type='checkbox' class='selecionar' title='Marcar / Desmarcar todos' style='float: left;'>" .
            							"<span class='qtde' style=' float: left; heigth: 36px; margin-left:3px;'></span>" .
            							"<input type='hidden' class='registros'>"
            							;

            	$filterValue = isset($filter['value']) ? $filter['value'] : $caixaDeVerificacao;
                        ...
                        if ($orderField == $title['field']) {
                            ...
                            if (!array_key_exists('url', $title)) {
                                ...
                                if ($this->getAlwaysShowOrderArrows() === true
									...
									*/
                                    // Estas classes estão em ../public/css/form.css (alterado para a direção certa, pois anteriormente estava trocado!)
									$link1 =	"<a href='" . $title['simpleUrl']
												. "/order{$this->getGridId()}/{$title['field']}_DESC'"
												. " class='ordenacaodesc' alt='Ordenação descendente' title='Ordenação descendente' ></a>";
									$link2 =	"<a href='" . $title['simpleUrl']
												. "/order{$this->getGridId()}/{$title['field']}_ASC'"
												. " class='ordenacaoasc' alt='Ordenação ascendente' title='Ordenação ascendente' ></a>";

                                    /* ASmR - Tradução ou alteração para compatibilização */
									if ($title['abbr']) {
										$abbrIni = "<abbr title='" . $title['abbr'] . "'>";
										$abbrFim = "</abbr>";
									} else {
										$abbrIni = "";
										$abbrFim = "";
                                    ...
                                    $grid .= $this->_temp['table']->titlesLoop($link2 . $title['value'] . $link1,
                                                                               $colspan);
                                    */
                                    $grid .= $this->_temp['table']->titlesLoop(	$link2 . $abbrIni . $title['value'] . $abbrFim . $link1,
                                    											$colspan);
                            } else {
                                    if ($this->getShowOrderImages() == false) {
                                        $hrefTitle = '';
    ...
    protected function _buildGridTable($grids)
    {
        ...
        if ($this->_totalRecords == 0) {
        	...
        	$grid = $this->_temp['table']->noResults($this->__('Nenhum registro encontrado!'));
        }
    ...
    protected function _buildPagination()
    {
        ...
        if (count($this->_paginationInterval) > 0 && $this->getTotalRecords() > 0) {
            ...
            $menuPerPage = ' | ' . $this->__('Show') . ' '
                         ...
                         . ' ' . $this->__('items');
            */
            $menuPerPage = '&nbsp;' . $this->__('Apresentar ') . '&nbsp;'
                         . $this->getView()->formSelect(
                            'perPage' . $this->getGridId(), $this->getParam('perPage', $this->_recordsPerPage), array('onChange' => "window.location='$url/perPage"
                         . $this->getGridId() . "/'+this.value;"), $this->_paginationInterval
                           )
                         . ' ' . $this->__('registros por página.');
        } else {
            $menuPerPage = '';
        }
		...
		************************************************************ */
        ...
        if (1 == $actual) {
            /* ASmR - Tradução ou alteração para compatibilização */
            /* $pag = "<strong> 1 </strong>"; */
            $pag = "<span class='disabled'> 1 </span>";
        } else {
        	/* ASmR - Tradução ou alteração para compatibilização */
        ...
        if ($npaginas > 5) {
            ...
            for ($i = $in + 1; $i < $fin; $i++) {
                ...
                if ($i == $actual) {
                    /* ASmR - Tradução ou alteração para compatibilização */
            		/* $pag .= "<strong> $i </strong>"; */
            		$pag .= "<span class='disabled'> $i </span>";
        ...
        if ($actual == $npaginas) {
        	...
        	$pag .= "<span class='disabled'> $npaginas </span>";
        } else {
            $pag .= "<a href=\"$url/start{$this->getGridId()}/"
        ...
        if ($actual != 1) {
            ...
            */
			$pag =	"<a href=\"$url/start/0\">" . $this->__('Primeira') . "</a>"
					. "<a href=\"$url/start{$this->getGridId()}/" . (($actual - 2) * $ppagina) . "\">" . $this->__('Anterior') . "</a>" . $pag;
        } /* ASmR - Tradução ou alteração para compatibilização */ else {
			$pag =	"<span class='disabled'>Primeira</span>" .
					"<span class='disabled'>Anterior</span>" . $pag;
        }
        ...
        if ($actual != $npaginas) {
            ...
            */
        	$pag .=	"<a href=\"$url/start{$this->getGridId()}/" . ($actual * $ppagina) . "\">" . $this->__('Próxima') . "</a>"
        			. "<a href=\"$url/start{$this->getGridId()}/" . (($npaginas - 1) * $ppagina) . "\">" . $this->__('Última') . "</a>";
        } /* ASmR - Tradução ou alteração para compatibilização */ else {
        	$pag .=	"<span class='disabled'>Próxima</span>" .
		...
		************************************************************ */
        if ($npaginas > 1 && $this->getInfo("limit") == 0) {
            ...
            // NÃO EXIBIR COMBO COM AS PÁGINAS DISPONÍVEIS!
            $pageSelect = '';
        }
        ...
        if ($npaginas > 1 || count($this->_export) > 0) {
			...
			************************************************************ */
			...
			$i = 0;
			$botoesAcao = '';
			// $massActionButtons = array();

			foreach ($this->getMassActions()->getMassActionsOptions() as $massAction) {
				// $massActionButtons[$i++] = "<a title='" . $massAction['caption'] . "' href='" . $massAction['url'] . "'><img src='/PHP/e-Admin/public/zfdg/images/zf_" . strtolower($massAction['caption']) . ".png' border='0'></a>";

			    $linkHref = " href='" . $massAction['url'] . "' ";
				switch ( strtolower ( $massAction ['imagem'] ) ) {
					case 'incluir':
					case 'importar':
						$classeAcao = "";
						break;
					case 'excluir':
						$classeAcao = "acaoExcluir";
						break;
					case 'restaurar':
						$classeAcao = "acaoRestaurar";
						break;
					case 'editar':
						$classeAcao = "acaoEditar";
						break;
					case 'editarcontrato':
						$classeAcao = "acaoEditarcontrato";
						break;
					case 'leitura':
						$classeAcao = "acaoLeitura";
						break;
					case 'detalhe':
						$classeAcao = "acaoVer";
						break;
				}
				$botoesAcao .= "<a class='$classeAcao' title='" . $massAction['caption'] . "' $linkHref >" . $images[strtolower($massAction['imagem'])] . "</a>" . PHP_EOL;
			}
            ...
            if ((int) $this->getInfo("limit") > 0) {
                ...
                */
				...
				$this->_temp['table']->pagination(
						'' . $pag,
						$this->__('Exibindo do registro <strong>') . $registoActual . $this->__('</strong> ao <strong>') . $registoFinal . $this->__('</strong>, num total de <strong>') . $this->_totalRecords . $this->__('</strong>.'),
						$menuPerPage,
						$pageSelect,
						$botoesAcao);
    ...
    public function deploy()
    {
        if ($this->getSource() === null) {
        	...
        	throw new Bvb_Grid_Exception($this->__('Favor informar a fonte de dados'));
        }
        ...
        if (((!$this->getParam('edit') )
            ...
            if ($this->_isDetail == true
                ...
                if ($this->getParam('delete')) {
                    ...
                    */
                    $buttonRemove = $this->getView()->formButton(
                    		'delRecordGrid', $this->__('Excluir registro'), array('onclick' => "window.location='$localDelete'")
                    );
                    ...
                    */
                    $buttonCancel = $this->getView()->formButton(
                    		'delRecordGrid', $this->__('Cancelar'), array('onclick' => "window.location='$localCancel'")
                    );
...
\$('document').ready(function() {
	...
	function atualiza() {
		...
		if ( \$('.chk:checked').length == 0) {
			...
			qtde = \$('.chk:checked').length;
		}

		\$('.selecionar').attr('checked', (\$('.chk:checked').length == \$('.chk').length) );
		\$('.qtde').html( qtde );
		// \$('.registros').html( selecionados.slice(0, -1) );
		\$('.registros').attr( 'value', selecionados );
	}

	\$('.acaoVer').click(function() {
		var qtde = \$('.chk:checked').length;

		if (qtde == 0) {
			alert('" . $this->__('Nenhum registro selecionado.') . "');
			return false;
		}

		if (qtde != 1) {
			alert('" . $this->__('Favor selecionar apenas 1 registro para essa ação!') . "');
			return false;
		}

		var registros = $('.registros').val();
		link = \$('.acaoVer').attr('href') + registros;
	...
	\$('.acaoEditar').click(function() {
        ...
        var registros = $('.registros').val();
        link = \$('.acaoEditar').attr('href') + registros;
		window.location = link;
		return false;
	});

	\$('.acaoEditarcontrato').click(function() {
		var qtde = \$('.chk:checked').length;
        ...
        var registros = $('.registros').val();
        link = \$('.acaoEditarcontrato').attr('href') + registros;
		window.location = link;
		return false;
	});
    ...
    \$('.acaoExcluir').click(function() {
		...
		var registros = $('.registros').val();
		link = \$('.acaoExcluir').attr('href') + registros;
		window.location = link;
		return false;
	});

	\$('.acaoLeitura').click(function() {
		var qtde = \$('.chk:checked').length;
        ...
        var registros = $('.registros').val();
        link = \$('.acaoLeitura').attr('href') + registros;
		window.location = link;
		return false;
	});

    \$('.acaoRestaurar').click(function() {
		var qtde = \$('.chk:checked').length;
		...
		var registros = $('.registros').val();
		link = \$('.acaoRestaurar').attr('href') + registros;
		window.location = link;
		return false;
	});

    $(document).delegate('table input[type=\"text\"]', 'keyup', function() {
        var value = $(this).val();
        //alert(value);
...
/* IDs dos campos selecionados: */
/* dados = postMassIds_" . $this->getGridId() . "; */
}

/* @author	Anderson Sathler */
function FORA_DE_USO___montaUrl(url, apenasUm = false)
{
	chkSelecionados = postMassIds_" . $this->getGridId() . ";
	//alert(chkSelecionados);
	...
	qtdeSelecionados = tempArray_" . $this->getGridId() . ".length;
	//alert(qtdeSelecionados);
	...
	if (apenasUm == true && qtdeSelecionados != 1) {
		alert('" . $this->__('Favor selecionar apenas 1 registro para essa ação!') . "');
		return false;
	}

	urlFinal = url + chkSelecionados;
...
function _" . $this->getGridId() . "gridChangeFilters_OLD(event)
    {
        ...
        for (var i = 0; i < fieldsArray.length -1; i++)
            {
                ...
                {
                    value = document.getElementById(fieldsArray[i]).attr( 'value' );
                }
                ...
                if ( value.length > 0 )
                {
                    ...
                    if ( name == '' || typeof(name) == 'undefined' ) {
                        name = document.getElementById(fieldsArray[i]).get('name');
                     }
...
function _" . $this->getGridId() . "gridChangeFilters(event)
    {
        ...
        $('table:visible:first th.filtro input[type=\"text\"]').each(function() {
            value = ($(this).val());
            if ($(this).val()) {
                value = value.replace(/^\s+|\s+$/g, '').replace(/\//, '%2F');
                name = ($(this).attr('name'));
    ...
    public function setForm(Bvb_Grid_Form $crud)
    {
        ...
        if ($crud->getBulkDelete() == true) {
            $this->getMassActions()->addMassActions(
                array(
                	...
                	*/
                    array('url' => $this->getUrl() . '/zfmassremove' . $this->getGridId() . '/1/',
                          'caption' => $this->__('Exclusão dos registros selecionados'),
                          'confirm' => $this->__('Você tem certeza?')
                    )
                )
            );
        }
        ...
        if ($crud->getBulkEdit() == true) {
            ...
            $this->getMassActions()->addMassActions(
                array(
                	...
                	*/
                    array('url' => $editMassUrl,
                          'caption' => $this->__('Edição dos registros selecionados')
                    )
                )
            );
        }
        ...
        if ($crud->getUseVerticalInputs() === false) {
            ...
            if ($formsCount > 1)
                ...
                $arr[0]->addElement('checkbox', 'ZFIGNORE', array('label' => $this->__('Ignorar'), 'order' => 0));
        ...
        for ($i = 1; $i <= $formsCount; $i++) {
            ...
            if ($formsCount > 1)
            	...
            	*/
            	$arr[$i]->addElement(
            		'checkbox', 'ZFIGNORE', array('label' => $this->__('Ignorar registro'), 'order' => 0)
            	);
            ...
            if (count($form->getElements()) == 0) {
            	...
            	throw new Bvb_Grid_Exception($this->__("Seu formulário não contém campos"));
            }
        ...
        if (isset($crud->options['saveAndAddButton'])
            ...
            $crud->getForm()->addElement(
            	...
            	*/
            	array('label' => $this->__('Salvar e novo'),
                      'class' => 'submit',
                      'decorators' => $crud->getButtonHiddenDecorator())
            );
        }
    ...
    protected function _formatField($field)
    {
        ...
        $attr['onKeyUp'] = "_" . $this->getGridId() . "gridChangeFilters(event);";
         ...
         /* ASmR - Tradução ou alteração para compatibilização */
         /*  Tratamento de valores e data na pesquisa
          *   Rodrigo Mariano
          */

        $propriedades = $this->_getColumn($field);
        if (array_key_exists('class', $propriedades)) {
            if ($propriedades['class'] == 'valorgrid') {
                $attr['class'] = "valorgrid";
            }
        }
...
//            }
    ...
    public function setImagesUrl($url)
    {
        if (!is_string($url)) {
        	...
        	throw new Bvb_Grid_Exception('Era esperado uma string; ' . gettype($url) . ' informado');
        }
        $this->_imagesUrl = $url;
        return $this;
    }
    ...
    protected function _buildMassActions()
    {
		...
		*/
        ...
        $decorator = "<input type='checkbox' class='chk' id='massCheckBox_" . $this->getGridId() . "' value='" . $pk . "' />";
        ...
        // Forma o combo <select> das massActions
        $select = array();
        foreach ($this->getMassActions()->getMassActionsOptions() as $value) {
        ...
        */
		$return =	"" .
					"<tr >" .
					"	<td " . $cssClasses['massActions'] . " colspan=" . $this->_colspan . ">" .
					"		<div style='text-align: right;'>" .
					"			<input type='checkbox' class='selecionar'> <a class='inverter'><img src='' height='13' width='13'></a> <a href='#' class='inverter'> Invert </a> | <span class='qtde'>0</span> <br />" . PHP_EOL . " <span class='registros'></span> " .
					/*
					"			<form style='padding:0;margin:0;' method='post' action='' id='massActions_{$this->getGridId()}' name='massActions_{$this->getGridId()}'>" .
					"				" . $this->getView()->formHidden('massActionsAll_' . $this->getGridId(), $ids) .
					"				" . $this->getView()->formHidden('postMassIds' . $this->getGridId(), '') . PHP_EOL .
					"				<span " . $cssClasses['massSelect'] . ">" .
					"					<a href='#' onclick='checkAll_" . $this->getGridId() . "(document.massActions_" . $this->getGridId() . ".gridMassActions_" . $this->getGridId() . ", {$this->getTotalRecords()},1);return false;'>" . $this->__('Selecionar todos') . "</a> | " . PHP_EOL .
					"					<a href='#' onclick='checkAll_" . $this->getGridId() . "(document.massActions_" . $this->getGridId() . ".gridMassActions_" . $this->getGridId() . ", {$currentRecords},0);return false;'>" . $this->__('Selecionar os visíveis') . "</a> | " . PHP_EOL .
					"					<a href='#' onclick='uncheckAll_" . $this->getGridId() . "(document.massActions_" . $this->getGridId() . ".gridMassActions_" . $this->getGridId() . ",0);return false;'>" . $this->__('Desselecionar todos') . "</a> | " . PHP_EOL .
					"					<strong><span id='massSelected_" . $this->getGridId() . "'>0</span></strong> " . $this->__('itens selecionados') . PHP_EOL .
					// "					" . $this->__('Ações') . ": " . PHP_EOL . "$formSelect $formSubmit " .
					"				</span>" .
					"" .
					"				<br />" . PHP_EOL .
					"" .
    ...
    public function setCrudColumnsPosition($positon = 'left')
    {
        if ($positon != 'left' && $positon != 'right') {
        	...
        	throw new Bvb_Grid_Exception('Favor escolher entre "left" ou "right"');
        }
