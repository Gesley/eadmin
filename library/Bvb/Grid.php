 ...
 * ********************************************************************************
 * @internal  Anderson Sathler M. Ribeiro <asathler@gmail.com>
 *            Tradução para pt-Br;
 *            Ajustes de funcionalidades (minor bugs or implementations);
 *            Adaptações para o layout do TRF1
 * ********************************************************************************
 */
...
abstract class Bvb_Grid {
    ...
    /* const VERSION = '0.8'; */
	const VERSION = '0.8 [+ASmR mods] [+pt-Br translation] [+TRF1 template mods]';
    /**
    ...
    public function __construct($options)
    {
        ...
        // Add the formatter dir for fields content
            ...
            foreach ($filters as $key => $filter) {
                ...
                if (!is_array($filter) && (strlen($filter) == 0 || !in_array($key, $this->_fields))) {
                    ...
                    if (is_array($this->_filters[$key])
                        ...
                        $propriedades = $this->_getColumn($key);
                        $valor = new Trf1_Orcamento_Valor();
                        $negativo = false;

                        if (array_key_exists('class', $propriedades)) {
                            if ($propriedades['class'] == 'valorgrid') {
                                $filter = $valor->retornaValorParaBancoRod($filter);
                                $centavos = explode('.', $filter);
                                if (isset($centavos[1])) {
                                    if ($centavos[1] == '00') {
                                        $filter = $centavos["0"];
                                    } else {
                                        $ultimocents = substr($centavos[1], -1);
                                        if ($ultimocents == "0") {
                                            $filter = $centavos[0] . "," . substr($centavos[1], 0, 1);
                                        }
                                    }
                                }
                                $real = explode(',', $filter);
                                if (($real[0] == "0") || ($real[0] == "00")) {
                                    if (isset($real[1])) {
                                        $filter = "," . $real[1];
                                    } else {
                                        $filter = "0";
    ...
    protected function _buildTitles()
    {
        ...
        for ($i = 0; $i < count($this->_fields); $i++) {
            ...
            if ($this->_displayField($titles[$i])) {
                ...
                /* ASmR - Tradução ou alteração para compatibilização */
                if (!isset($this->_data['fields'][$titles[$i]]['abbr'])) {
                    $abbr = null;
                } else {
                    $abbr = $this->_data['fields'][$titles[$i]]['abbr'];
                }
        ...
        foreach ($this->getExtraColumnsForPosition('right') as $key => $value) {
            ...
            $value['rowspan'] = !isset($value['rowspan']) ? null : $value['rowspan'];
            $value['colspan'] = !isset($value['colspan']) ? null : $value['colspan'];
            $value['abbr'] = !isset($value['abbr']) ? false : $value['abbr'];
    ...
    public function checkExportRights()
    {
        if (!in_array($this->_deployName, $this->_export) && !array_key_exists($this->_deployName, $this->_export)) {
        	...
        	throw new Bvb_Grid_Exception($this->__("Você não possui permissão para exportação de dados para este formato"));
        }
    }
