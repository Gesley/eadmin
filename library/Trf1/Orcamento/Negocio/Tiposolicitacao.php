...
/**
 * @category	TRF1
 * @package		Trf1_Orcamento_Negocio_Tiposolicitacao
 ...
 * 
 * TRF1, Classe negocial sobre Orçamento - Tipos de solicitações
 * 
 * ====================================================================================================
 * LICENSA
 * ====================================================================================================
 ...
 * 
 */
class Trf1_Orcamento_Negocio_Tiposolicitacao {
	/**
	...
	public function __construct() {
		$this->_dados = new Application_Model_DbTable_Orcamento_CeoTbTsolTipoSolicitacao ();
	}
	...
	public function retornaCombo() {
		...
		$cacheId = $cache->retornaID_Combo ( 'tiposolicitacao' );
		...
		if ($dados === false) {
			...
			$sql = "
SELECT
	TSOL_CD_TIPO_SOLICITACAO,
	TSOL_DS_TIPO_SOLICITACAO
FROM
	CEO_TB_TSOL_TIPO_SOLICITACAO
WHERE
	TSOL_DH_EXCLUSAO_LOGICA IS NULL
ORDER BY
	TSOL_CD_TIPO_SOLICITACAO ASC
					";
	...
	public function retornaListagem() {
		// Verifica existência dos dados em cache
		$cache = new Trf1_Orcamento_Cache ();
		$cacheId = $cache->gerarID_Listagem ( 'tiposolicitacao' );
		$dados = $cache->lerCache ( $cacheId );
...
FROM
	CEO_TB_TSOL_TIPO_SOLICITACAO
WHERE
	TSOL_CD_TIPO_SOLICITACAO					= $tipo		AND
	TSOL_DH_EXCLUSAO_LOGICA						IS NULL
				";
	...
	public function retornaRegistroNomeAmigavel($tipo) {
		$sql = "
SELECT
	TSOL_CD_TIPO_SOLICITACAO					AS \"Tipo de solicitação\",
	TSOL_DS_TIPO_SOLICITACAO					AS \"Descrição\"
FROM
	CEO_TB_TSOL_TIPO_SOLICITACAO
WHERE
	TSOL_CD_TIPO_SOLICITACAO					= $tipo		AND
...
FROM
	CEO_TB_TSOL_TIPO_SOLICITACAO
WHERE
	TSOL_CD_TIPO_SOLICITACAO					IN ($tipos)		AND
	TSOL_DH_EXCLUSAO_LOGICA						IS NULL
				";
...
UPDATE
	CEO_TB_TSOL_TIPO_SOLICITACAO
SET
	TSOL_CD_MATRICULA_EXCLUSAO					= '$sessao->matricula',
	TSOL_DH_EXCLUSAO_LOGICA						= SYSDATE
WHERE
	TSOL_CD_TIPO_SOLICITACAO					IN ($tipos)		AND
	TSOL_DH_EXCLUSAO_LOGICA						IS NULL
	";
