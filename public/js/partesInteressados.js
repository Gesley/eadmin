            ...
            $("#PAPD_CD_MATRICULA_INTERESSADO").autocomplete({
                ...
                select: function(event, ui){
                    var config = $.data(document.body,'config');
                     ...
                     existe_na_lista = config.containerPartes.find("input[value="+ui.item.matricula+"-"+ui.item.id+"-"+config.tipoParte+"]");  
                     ...
                     if(encontrou != undefined){
                        ...
                        tr += "<td style='width: 5%'><a href='#' class='remover-parte' rel='"+ui.item.matricula+"-"+ui.item.id+"-"+config.tipoParte+"' >Remover</a></td>";
                        tr += "<td>"+ ui.item.label +" </td>";
                        tr += "<input type='hidden' value='"+ui.item.matricula+"-"+ui.item.id+"-"+config.tipoParte+"' name='partes_pessoa_trf[]' />";
                        tr += "</tr>",
