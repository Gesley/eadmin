 ...
 *
 * @category Orcamento
 * @package Orcamento_ImportarprovisaoController
 * @author Ricardo Ramon <rrsantos2@stefanini.com>
 ...
 */
class Orcamento_ImportarprovisaoController extends Orcamento_Business_Tela_Crud {

    private $_titulo = "Provisao";
    private $_padrao = ImportBuffer_Constants::PADRAO1;
    private $_tipo = Orcamento_Business_Importacao_Base::ARQUIVO_PROVISAO;
    ...
    public function init() {
        ...
        $this->defineTituloBrowser("Importar Provisão");

        // Define a classe facade
        $this->defineFacade('Importar' . strtolower($this->_titulo));

        // Inicia a negocio
        $this->_negocioProvisao = new Orcamento_Business_Negocio_Importarprovisao ();
    ...
    public function indexAction() {
        ...
        $funcionalidade = "Importar Provisão";
    ...
    {
        // Título da tela (action)
        $this->view->telaTitle = 'Editar Provisão';

        $formulario = new Orcamento_Form_Importarprovisao ();
        $this->view->formulario = $formulario;
        
        $cod = $this->_getParam('cod');
        if ($this->getRequest()->isPost()) {
            $dados = $this->getRequest()->getPost();
            $res = $this->_negocioProvisao->editar($dados);
            if($res == FALSE){
    ...
    public function incluirAction ()
    {
        ...
        // Título da tela (action)
        $this->view->telaTitle = 'Incluir Provisão';        
    ...
    public function importarAction() {
        ...
        // Título da tela (action)
        $this->view->titulo = 'Importar provisão';
        ...
        try {
            ...
            if ($this->_request->isPost()) {
                ...
                // model de provisao
                $negocioprovisao = new Orcamento_Business_Negocio_Importarprovisao ();
                // model de importacao
                ...
                $arrayprovisao = $negocioImportacao->retornaPadrao1($arquivo);

                // Valida o arquivo txt enviado
                $validacao = $negocioImportacao->validaArray($arrayprovisao);              
                if($validacao == FALSE ){
                ...
                // Inclui
                foreach ($arrayprovisao as  $provisao) {
                    if( $resImportacao['sucesso'] ){
                        // verifica se já existe se sim deleta
                        $existe = $negocioImportacao->verificaExistentes( $provisao, $p["IMPA_AA_IMPORTACAO"], $this->_tipo );
                        // tabela de valores
                        $res = $negocioprovisao->incluirDados( $provisao, $resImportacao['codigo'] );
    ...
    {
        // Título da tela (action)
        $this->view->telaTitle = 'Excluir Provisao';
        ...
        if ($this->getRequest()->isGet()) {      
            ...
            if ($chavePrimaria) {
                    ...
                    $this->view->codigo = $this->_negocioProvisao->chavePrimaria();
