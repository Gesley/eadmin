...
/**
 * Disponibiliza importar suplementacao.
 *
 * @category Orcamento
 * @package Orcamento_ImportarsuplementacaoController
 * @author Sandro Maceno <smaceno@stefanini.com>
 ...
 */
class Orcamento_ImportarsuplementacaoController extends Orcamento_Business_Tela_Crud {

    private $_tipo = Orcamento_Business_Importacao_Base::ARQUIVO_SUPLEMENTACAO;
    ...
    public function init() {
        ...
        $this->defineTituloBrowser('Importação Suplementação');

        // Define a classe facade
        $this->defineFacade('Importarsuplementacao');

        // Inicia a negocio
        $this->_negocioSuplementacao = new Orcamento_Business_Negocio_ImportarSuplementacao ();
    ...
    public function incluirAction ()
    {
        ...
        $this->view->telaTitle = 'Incluir Suplementação';
    ...
    public function importarAction() {
        ...
        // Título da tela (action)
        $this->view->titulo = 'Importar Suplementação';
        ...
        try {
            ...
            if ($this->_request->isPost()) {
                ...
                // model de suplementacao
                $negocioSuplementacao = new Orcamento_Business_Negocio_ImportarSuplementacao ();
                // model de importacao
                ...
                $arraysuplementacao = $negocioImportacao->retornaPadrao1($arquivo);

                // Valida o arquivo txt enviado
                $validacao = $negocioImportacao->validaArray($arraysuplementacao);              
                if($validacao == FALSE ){
                    $this->erroOperacao("Arquivo corrompido, não foi possível finalizar importação !");
                    $this->_redirect( 'orcamento/'.$this->_controller.'/importar');
                } 
                ...
                // tabela de importacao -> nome do arquivo e ano
                $resImportacao = $negocioImportacao->incluirDados( $f['IMPA_DS_ARQUIVO']['name'], $p['IMPA_AA_IMPORTACAO'], 2 );

                // Inclui
                foreach ($arraysuplementacao as  $suplementacao) {
                    if( $resImportacao['sucesso'] ){
                        // verifica se já existe se sim deleta
                        $existe = $negocioImportacao->verificaExistentes( $suplementacao, $p["IMPA_AA_IMPORTACAO"], $this->_tipo );
                        // tabela de valores
                        $res = $negocioSuplementacao->incluirDados( $suplementacao, $resImportacao['codigo'] );
                ...
                $this->_helper->flashMessenger(array(message => Trf1_Orcamento_Definicoes::MENSAGEM_INCLUIR_SUCESSO, 'status' => 'success'));
                // redirect 
                $this->_redirect( 'orcamento/importarsuplementacao/index');
    ...
    {
        // Título da tela (action)
        $this->view->telaTitle = 'Editar Suplementação';

        $formulario = new Orcamento_Form_Importarsuplementacao ();
        ...
        if ($this->getRequest()->isPost()) {
            $dados = $this->getRequest()->getPost();
            $res = $this->_negocioSuplementacao->editar($dados);
            if($res == FALSE){
                $this->erroOperacao("Não foi possível completar a ação, pois se trata de um registro duplicado.");
    ...
    {
        // Título da tela (action)
        $this->view->telaTitle = 'Excluir Suplementação';
        ...
        if ($this->getRequest()->isGet()) {      
            ...
            if ($chavePrimaria) {
                    ...
                    $this->view->codigo = $this->_negocioSuplementacao->chavePrimaria();
