 ...
 *
 * @category Orcamento
 * @package Orcamento_ImportarrestosapagarController
 * @author Ricardo Ramon <rrsantos2@stefanini.com>
 ...
 */
class Orcamento_ImportarrestosapagarController extends Orcamento_Business_Tela_Crud {

    private $_titulo = "Restosapagar";
    private $_padrao = ImportBuffer_Constants::PADRAO2;
    private $_tipo = Orcamento_Business_Importacao_Base::ARQUIVO_RESTOSAPAGAR;
    ...
    public function init() {
        ...
        $this->defineTituloBrowser("Importar Restos A Pagar");

        // Define a classe facade
        $this->defineFacade('Importar' . strtolower($this->_titulo));

        // Inicia a negocio
        $this->_negocioRestos = new Orcamento_Business_Negocio_Importarrestosapagar ();
    ...
    public function indexAction() {
        ...
        $funcionalidade = "Importar Restos A Pagar";
    ...
    {
        // Título da tela (action)
        $this->view->telaTitle = 'Editar Restos a pagar';

        $formulario = new Orcamento_Form_Importarempenhado ();
        $this->view->formulario = $formulario;
        
        $cod = $this->_getParam('cod');
        if ($this->getRequest()->isPost()) {
            $dados = $this->getRequest()->getPost();
            $res = $this->_negocioRestos->editar($dados);
            if($res == FALSE){
    ...
    public function incluirAction ()
    {
        ...
        // Título da tela (action)
        $this->view->telaTitle = 'Incluir restos a pagar';
     ...
     * @update Gesley Rodrigues
     */
    public function importarAction() {
        
        // Título da tela (action)
        $this->view->titulo = 'Importar restos a pagar';
        ...
        try {
            ...
            if ($this->_request->isPost()) {
                ...
                // model de restosapagar
                $negociorestosapagar = new Orcamento_Business_Negocio_Importarrestosapagar ();
                // model de importacao
                ...
                $arrayrestosapagar = $negocioImportacao->retornaPadrao2($arquivo);
                
                // Valida o arquivo txt enviado
                $validacao = $negocioImportacao->validaArray($arrayrestosapagar);              
                if($validacao == FALSE ){
                ...
                // Inclui
                foreach ($arrayrestosapagar as  $restosapagar) {
                    if( $resImportacao['sucesso'] ){
                        // verifica se já existe se sim deleta
                        $negocioImportacao->verificaExistentes( $restosapagar, $p["IMPA_AA_IMPORTACAO"], $this->_tipo );
                        // tabela de valores
                        $res = $negociorestosapagar->incluirDados( $restosapagar, $resImportacao['codigo'] );
                    }                
                }
    ...
    {
        // Título da tela (action)
        $this->view->telaTitle = 'Excluir Restos a pagar';
        ...
        if ($this->getRequest()->isGet()) {      
            ...
            if ($chavePrimaria) {
                    ...
                    $this->view->codigo = $this->_negocioRestos->chavePrimaria();
