 ...
 *
 * @category Orcamento
 * @package Orcamento_ImportarempenhadoController
 * @author Ricardo Ramon <rrsantos2@stefanini.com>
 ...
 */
class Orcamento_ImportarempenhadoController extends Orcamento_Business_Tela_Crud {

    private $_titulo = "Empenhado";
    ...
    public function init() {
        ...
        // Inicia a negocio
        $this->_negocioEmpenhado = new Orcamento_Business_Negocio_Importarempenhado ();
    ...
    {
        // Título da tela (action)
        $this->view->telaTitle = 'Editar Empenhado';
        ...
        if ($this->getRequest()->isPost()) {
            ...
            $res = $this->_negocioEmpenhado->editar($dados);
            if($res == FALSE){
    ...
    public function incluirAction ()
    {
        ...
        // Título da tela (action)
        $this->view->telaTitle = 'Incluir empenhado';
    ...
    public function importarAction() {
        ...
        // Título da tela (action)
        $this->view->titulo = 'Importar empenhado';
        ...
        try {
            ...
            if ($this->_request->isPost()) {
                ...
                // model de empenhado
                $negocioempenhado = new Orcamento_Business_Negocio_Importarempenhado ();
                // model de importacao
                ...
                $arrayempenhado = $negocioImportacao->retornaPadrao2($arquivo);

                // Valida o arquivo txt enviado
                $validacao = $negocioImportacao->validaArray($arrayempenhado);              
                if($validacao == FALSE ){
                ...
                // Inclui
                foreach ($arrayempenhado as  $empenhado) {
                    if( $resImportacao['sucesso'] ){
                        // verifica se já existe se sim deleta
                        $existe = $negocioImportacao->verificaExistentes( $empenhado, $p["IMPA_AA_IMPORTACAO"], $this->_tipo );
                        // tabela de valores
                        $res = $negocioempenhado->incluirDados( $empenhado, $resImportacao['codigo'] );
    ...
    {
        // Título da tela (action)
        $this->view->telaTitle = 'Excluir Empenhado';
        ...
        if ($this->getRequest()->isGet()) {      
            ...
            if ($chavePrimaria) {
                    ...
                    $this->view->codigo = $this->_negocioEmpenhado->chavePrimaria();
