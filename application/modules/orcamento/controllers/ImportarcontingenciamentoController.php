...
/**
 * Disponibiliza importar contingenciamento.
 *
 * @category Orcamento
 * @package Orcamento_ImportarcontingenciamentoController
 * @author Sandro Maceno <smaceno@stefanini.com>
 ...
 */
class Orcamento_ImportarcontingenciamentoController 
        extends Orcamento_Business_Tela_Crud {

    private $_tipo = Orcamento_Business_Importacao_Base::ARQUIVO_CONTINGENCIAMENTO;
    ...
    public function init() {
        ...
        $this->defineTituloBrowser('Importar Contingenciamento');

        // Define a classe facade
        $this->defineFacade('Importarcontingenciamento');

        // Inicia a negocio
        $this->_negocioContingenciamento = new Orcamento_Business_Negocio_ImportarContingenciamento ();
    ...
    public function indexAction() {
        ...
        $funcionalidade = 'Importar Contingenciamento';

        // remove cache
        $cache = new Trf1_Cache();
        $controller = Zend_Controller_Front::
                getInstance()->getRequest()->getControllerName();
    ...
    {
        // Título da tela (action)
        $this->view->telaTitle = 'Incluir Contingenciamento';
    ...
    public function importarAction() {
        ...
        // Título da tela (action)
        $this->view->titulo = 'Importar Contingenciamento';
        ...
        try {
            ...
            if ($this->_request->isPost()) {
                ...
                // model de contingenciamento
                $negociocontingenciamento = new Orcamento_Business_Negocio_Importarcontingenciamento ();
                // model de importacao
                ...
                $arraycontingenciamento = $negocioImportacao->retornaPadrao1($arquivo);

                // Valida o arquivo txt enviado
                $validacao = $negocioImportacao->validaArray($arraycontingenciamento);              
                if($validacao == FALSE ){
                ...
                // Inclui
                foreach ($arraycontingenciamento as  $contingenciamento) {
                    if( $resImportacao['sucesso'] ){
                        // verifica se já existe se sim deleta
                        $existe = $negocioImportacao->verificaExistentes( $contingenciamento, $p["IMPA_AA_IMPORTACAO"], $this->_tipo );
                        // tabela de valores
                        $res = $negociocontingenciamento->incluirDados( $contingenciamento, $resImportacao['codigo'] );
                    }                
                }
    ...
    {
        // Título da tela (action)
        $this->view->telaTitle = 'Editar Contingenciamento';

        $formulario = new Orcamento_Form_Importarcontingenciamento ();
        $this->view->formulario = $formulario;
        
        $cod = $this->_getParam('cod');
        if ($this->getRequest()->isPost()) {
            $dados = $this->getRequest()->getPost();
            $res = $this->_negocioContingenciamento->editar($dados);
            if($res == FALSE){
                $this->erroOperacao("Não foi possível completar a ação, pois se trata de um registro duplicado.");
    ...
    {
        // Título da tela (action)
        $this->view->telaTitle = 'Excluir Contingenciamento';
        ...
        if ($this->getRequest()->isGet()) {      
            ...
            if ($chavePrimaria) {
                    ...
                    $this->view->codigo = $this->_negocioContingenciamento->chavePrimaria();
