...
/**
 * Disponibiliza as funcionalidades ao usuário sobre períodos de travamento da
 * projeção.
 *
 * @category Orcamento
 * @package Orcamento_TravaprojecaoController
 * @author Anderson Sathler M. Ribeiro <asathler@gmail.com>
 ...
 */
class Orcamento_TravaprojecaoController extends Zend_Controller_Action {
	/**
	...
	public function init() {
		...
		$this->view->title = 'Travamento da projeção';
	...
	public function indexAction() {
		...
		$this->view->telaTitle = 'Listagem dos travamentos da projeção';
		...
		$camposDetalhes = array (	'TRVP_CD_UG' => array ('title' => 'UG', 'abbr' => '' ),
									'UNGE_SG_UG' => array ('title' => 'Sigla UG', 'abbr' => '' ),
									'DT_INICIO' => array ('title' => 'Início do bloqueio', 'abbr' => '' ),
									'DT_FIM' => array ('title' => 'Término do bloqueio', 'abbr' => '' )
								);
		$camposOcultos = array ( 'TRVP_DT_INICIO', 'EXERCICIO' );
	...
	public function incluirAction() {
		// Título da tela (action)
		$this->view->telaTitle = 'Incluir travamento da projeção';
		...
		if ($this->getRequest ()->isPost ()) {
			...
			if ($formulario->isValid ( $dados )) {
                ...
                $dados["TRVP_DT_INICIO"] =  new Zend_Db_Expr("TO_DATE('".$dados['TRVP_DT_INICIO']."','DD/MM/YYYY')");
                $dados["TRVP_DT_FIM"] =  new Zend_Db_Expr("TO_DATE('".$dados['TRVP_DT_FIM']."','DD/MM/YYYY')");
	...
	public function detalheAction() {
		// Título da tela (action)
		$this->view->telaTitle = 'Visualizar travamento da projeção';
	...
	public function editarAction() {
		// Título da tela (action)
		$this->view->telaTitle = 'Editar travamento da projeção';
		...
		if ($this->getRequest ()->isGet ()) {
			...
			if ($formulario->isValid ( $dados )) {
                ...
                $trava = new Application_Model_DbTable_Orcamento_CeoTbTrvpTravaProjecao();
                $chavePrimaria[1] =  new Zend_Db_Expr("TO_DATE('$chavePrimaria[1]','YYYY-MM-DD')");
                $registro = $trava->find ( $chavePrimaria[0],  $chavePrimaria[1] )->current ();
                $dados["TRVP_DT_FIM"] =  new Zend_Db_Expr("TO_DATE('".$dados['TRVP_DT_FIM']."','DD/MM/YYYY')");
	...
	public function excluirAction() {
		// Título da tela (action)
		$this->view->telaTitle = 'Excluir travamentos da projeção';
				...
				if ($chavePrimaria) {
					...
					try {
						foreach ($chaves as $chave) {
							$chaveComposta = explode('-',$chave);
							
							// Acerta o parâmetro de data
							$chaveComposta [1] = substr ( $chaveComposta [1], 6, 2 ) . '/' . substr ( $chaveComposta [1], 4, 2 ) . '/' . substr ( $chaveComposta [1], 0, 4 );
							
							// Busca registro pela chave primária
							$registro = $tabela->find ( $chaveComposta[0], $chaveComposta[1] )->current ();
							
							if ($registro) {
								// Grava as alterações no banco
								$codigo = $registro->delete ();
                                                                // inclui na tabela de log do orçamento
