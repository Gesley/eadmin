...
/**
 * Disponibiliza importar pago.
 *
 * @category Orcamento
 * @package Orcamento_ImportarpagoController
 * @author Sandro Maceno <smaceno@stefanini.com>
 ...
 */
class Orcamento_ImportarpagoController extends Orcamento_Business_Tela_Crud {


    private $_tipo = Orcamento_Business_Importacao_Base::ARQUIVO_PAGO;
    ...
    public function init() {
        ...
        $this->defineTituloBrowser('Importação Pago');

        // Define a classe facade
        $this->defineFacade('Importarpago');

        // Inicia a negocio
        $this->_negocioPago = new Orcamento_Business_Negocio_ImportarPago ();
    ...
    public function indexAction() {
        ...
        $funcionalidade = 'Importação Pago';
    ...
    {
        // Título da tela (action)
        $this->view->telaTitle = 'Editar Pago';

        $formulario = new Orcamento_Form_Importarpago ();
        $this->view->formulario = $formulario;
        
        $cod = $this->_getParam('cod');
        if ($this->getRequest()->isPost()) {
            $dados = $this->getRequest()->getPost();
            $res = $this->_negocioPago->editar($dados);
            if($res == FALSE){
    ...
    public function incluirAction ()
    {
        ...
        // Título da tela (action)
        $this->view->telaTitle = 'Incluir pago';
    ...
    public function importarAction() {
        ...
        // Título da tela (action)
        $this->view->titulo = 'Importar pago';
        ...
        try {
            ...
            if ($this->_request->isPost()) {
                ...
                // model de pago
                $negociopago = new Orcamento_Business_Negocio_ImportarPago ();
                // model de importacao
                ...
                $arraypago = $negocioImportacao->retornaPadrao2($arquivo);

                // Valida o arquivo txt enviado
                $validacao = $negocioImportacao->validaArray($arraypago);              
                if($validacao == FALSE ){
                ...
                // Inclui
                foreach ($arraypago as  $pago) {
                    if( $resImportacao['sucesso'] ){
                        // verifica se já existe se sim deleta
                        $negocioImportacao->verificaExistentes( $pago, $p['IMPA_AA_IMPORTACAO'], $this->_tipo );
                        // tabela de valores
                        $negociopago->incluirDados( $pago, $resImportacao['codigo'] );
    ...
    {
        // Título da tela (action)
        $this->view->telaTitle = 'Excluir Pago';
        ...
        if ($this->getRequest()->isGet()) {      
            ...
            if ($chavePrimaria) {
                    ...
                    $this->view->codigo = $this->_negocioPago->chavePrimaria();
