 ...
 *
 * @category Orcamento
 * @package Orcamento_ImportarcancelamentoController
 * @author Ricardo Ramon <rrsantos2@stefanini.com>
 ...
 */
class Orcamento_ImportarcancelamentoController extends Orcamento_Business_Tela_Crud {

    private $_titulo = "Cancelamento";
    private $_padrao = ImportBuffer_Constants::PADRAO1;
    private $_tipo = Orcamento_Business_Importacao_Base::ARQUIVO_CANCELAMENTO;
    ...
    public function init() {
        ...
        // Inicia a negocio
        $this->_negocioCancelamento = new Orcamento_Business_Negocio_Importarcancelamento ();        
    ...
    public function incluirAction ()
    {
        ...
        // Título da tela (action)
        $this->view->telaTitle = "Incluir {$this->_titulo}";
    ...
    public function importarAction() {
        ...
        // Título da tela (action)
        $this->view->titulo = 'Importar Cancelamento';
        ...
        try {
            ...
            if ($this->_request->isPost()) {
                ...
                // model de cancelamento
                $negociocancelamento = new Orcamento_Business_Negocio_Importarcancelamento ();
                // model de importacao
                ...
                $arraycancelamento = $negocioImportacao->retornaPadrao1($arquivo);
                
                // Valida o arquivo txt enviado
                $validacao = $negocioImportacao->validaArray($arraycancelamento);              
                if($validacao == FALSE ){
                ...
                // Inclui
                foreach ($arraycancelamento as  $cancelamento) {
                    if( $resImportacao['sucesso'] ){
                        // verifica se já existe se sim deleta
                        $negocioImportacao->verificaExistentes( $cancelamento, $p["IMPA_AA_IMPORTACAO"], $this->_tipo );
                        // tabela de valores
                        $negociocancelamento->incluirDados( $cancelamento, $resImportacao['codigo'] );
    ...
    {
        // Título da tela (action)
        $this->view->telaTitle = 'Editar Cancelamento';

        $formulario = new Orcamento_Form_Importarcancelamento ();
        $this->view->formulario = $formulario;
        
        $cod = $this->_getParam('cod');
        if ($this->getRequest()->isPost()) {
            $dados = $this->getRequest()->getPost();
            $res = $this->_negocioCancelamento->editar($dados);
            if($res == FALSE){
                $this->erroOperacao("Não foi possível completar a ação, pois se trata de um registro duplicado.");
    ...
    {
        // Título da tela (action)
        $this->view->telaTitle = 'Excluir Cancelamento';
        ...
        if ($this->getRequest()->isGet()) {      
            ...
            if ($chavePrimaria) {
                    ...
                    $this->view->codigo = $this->_negocioCancelamento->chavePrimaria();
