 ...
 *
 * @category Orcamento
 * @package Orcamento_ImportarFinanceiroController
 * @author Ricardo Ramon <rrsantos2@stefanini.com>
 ...
 */
class Orcamento_ImportarfinanceiroController extends Orcamento_Business_Tela_Crud {

    private $_titulo = "Financeiro";
    private $_padrao = ImportBuffer_Constants::PADRAO3;
    private $_tipo = Orcamento_Business_Importacao_Base::ARQUIVO_FINANCEIRO;
    ...
    public function init() {
        ...
        // Inicia a negocio
        $this->_negocioFinanceiro = new Orcamento_Business_Negocio_Importarfinanceiro ();
    ...
    public function incluirAction ()
    {
        ...
        $this->view->telaTitle = 'Incluir Financeiro';
        
        if ($this->getRequest()->isPost()) {
            try {
                $dados = $this->getRequest()->getPost();

                if($dados['IMPA_AA_IMPORTACAO'] > date('Y')){
                ...
                $negocioImportacao->verificaExistentes( $dados );
    ...
    {
        // Título da tela (action)
        $this->view->telaTitle = 'Editar Financeiro';

        $formulario = new Orcamento_Form_Importarfinanceiro ();
        $this->view->formulario = $formulario;
        
        $cod = $this->_getParam('cod');
        if ($this->getRequest()->isPost()) {
            $dados = $this->getRequest()->getPost();
            $res = $this->_negocioFinanceiro->editar($dados);
            if($res == FALSE){
                $this->erroOperacao("Não foi possível completar a ação, pois se trata de um registro duplicado.");
    ...
    {
        // Título da tela (action)
        $this->view->telaTitle = 'Excluir Financeiro';
        ...
        if ($this->getRequest()->isGet()) {      
            ...
            if ($chavePrimaria) {
                    ...
                    $this->view->codigo = $this->_negocioFinanceiro->chavePrimaria();
    ...
    public function importarAction() {
        ...
        // Título da tela (action)
        $this->view->titulo = 'Importar Financeiro';
        ...
        try {
            ...
            // Título da tela (action)
            $this->view->telaTitle = 'Importar Financeiro';
            ...
            if ($this->_request->isPost()) {
                ...
                // model de financeiro
                $negocioFinanceiro = new Orcamento_Business_Negocio_Importarfinanceiro ();
                // model de importacao
                $negocioImportacao = new Orcamento_Business_Negocio_ImpaImportar ();
                ...
                $arrayFinanceiro = $negocioImportacao->retornaPadrao3($arquivo);
                

                
                // Valida o arquivo txt enviado
                $validacao = $negocioImportacao->validaArray($arrayFinanceiro);              
                if($validacao == FALSE ){
                ...
                // Inclui
                foreach ($arrayFinanceiro as  $financeiro) {
                    // verifica se já existe se sim deleta ( dados / ano )
                    $negocioImportacao->verificaExistentes( $financeiro, $p["IMPA_AA_IMPORTACAO"], $this->_tipo );

                    // tabela de valores
                    $negocioFinanceiro->incluirDados( $financeiro, $resImportacao['codigo'] );
                }
    ...
    public function removeralinincisoAction()
    {
        $cod = $this->_getParam('cod');       
        
        $negocio = new Orcamento_Business_Negocio_Importarfinanceiro();                
        $negocio->excluirIncisoAlinea($cod);
