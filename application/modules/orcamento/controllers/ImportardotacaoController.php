...
/**
 * Disponibiliza importar dotacao.
 *
 * @category Orcamento
 * @package Orcamento_ImportardotacaoController
 * @author Sandro Maceno <smaceno@stefanini.com>
 ...
 */
class Orcamento_ImportardotacaoController extends Orcamento_Business_Tela_Crud {

    private $_tipo = Orcamento_Business_Importacao_Base::ARQUIVO_DOTACAO;
    ...
    public function init() {
        ...
        $this->defineTituloBrowser('Importar Dotação Inicial');

        // Define a classe facade
        $this->defineFacade('Importardotacao');

        // Inicia a negocio
        $this->_negocioDotacao = new Orcamento_Business_Negocio_ImportarDotacao ();
    ...
    public function indexAction() {
        ...
        $funcionalidade = 'Importar Dotação Inicial';
    ...
    {
        // Título da tela (action)
        $this->view->telaTitle = 'Incluir Dotação';
    ...
    public function importarAction() {
        ...
        // Título da tela (action)
        $this->view->titulo = 'Importar Dotação';
        ...
        try {
            ...
            if ($this->_request->isPost()) {
                ...
                // model de dotacao
                $negociodotacao = new Orcamento_Business_Negocio_ImportarDotacao ();
                // model de importacao
               ...
               // Verifica se o ano é maior do que o atual
                ...
                $arraydotacao = $negocioImportacao->retornaPadrao1($arquivo);

                // Valida o arquivo txt enviado
                $validacao = $negocioImportacao->validaArray($arraydotacao);
                ...
                // Inclui
                foreach ($arraydotacao as  $dotacao) {
                    if( $resImportacao['sucesso'] ){
                        // verifica se já existe se sim deleta
                        $negocioImportacao->verificaExistentes( $dotacao, $p["IMPA_AA_IMPORTACAO"], $this->_tipo );
                        // tabela de valores
                        $negociodotacao->incluirDados( $dotacao, $resImportacao['codigo'] );
    ...
    {
        // Título da tela (action)
        $this->view->telaTitle = 'Editar Dotação';
        ...
        if ($this->getRequest()->isPost()) {
            $dados = $this->getRequest()->getPost();
            $res = $this->_negocioDotacao->editar($dados);
            if($res == FALSE){
                $this->erroOperacao("Não foi possível completar a ação, pois se trata de um registro duplicado.");
    ...
    {
        // Título da tela (action)
        $this->view->telaTitle = 'Excluir Dotação';
        ...
        if ($this->getRequest()->isGet()) {      
            ...
            if ($chavePrimaria) {
                    ...
                    $this->view->codigo = $this->_negocioDotacao->chavePrimaria();
