...
/**
 * Disponibiliza importar destaque.
 *
 * @category Orcamento
 * @package Orcamento_ImportardestaqueController
 * @author Sandro Maceno <smaceno@stefanini.com>
 ...
 */
class Orcamento_ImportardestaqueController 
        extends Orcamento_Business_Tela_Crud {

    private $_tipo = Orcamento_Business_Importacao_Base::ARQUIVO_DESTAQUE;
    ...
    public function init() {
        ...
        $this->defineTituloBrowser('Importar Destaque');

        // Define a classe facade
        $this->defineFacade('Importardestaque');

        // Inicia a negocio
        $this->_negocioDestaque = new Orcamento_Business_Negocio_ImportarDestaque ();
    ...
    public function indexAction() {
        ...
        $funcionalidade = 'Importar Destaque';
    ...
    {
        // Título da tela (action)
        $this->view->telaTitle = 'Editar Destaque';

        $formulario = new Orcamento_Form_Importardestaque ();
        $this->view->formulario = $formulario;
        
        $cod = $this->_getParam('cod');
        if ($this->getRequest()->isPost()) {
            $dados = $this->getRequest()->getPost();
            $res = $this->_negocioDestaque->editar($dados);
            if($res == FALSE){
    ...
    public function incluirAction ()
    {
        ...
        // Título da tela (action)
        $this->view->telaTitle = 'Incluir Destaque';
    ...
    public function importarAction() {
        ...
        // Título da tela (action)
        $this->view->titulo = 'Importar destaque';
        ...
        try {
            ...
            if ($this->_request->isPost()) {
                ...
                // model de destaque
                $negociodestaque = new Orcamento_Business_Negocio_ImportarDestaque ();
                // model de importacao
                ...
                $arraydestaque = $negocioImportacao->retornaPadrao1($arquivo);

                // Valida o arquivo txt enviado
                $validacao = $negocioImportacao->validaArray($arraydestaque);              
                if($validacao == FALSE ){
                ...
                // Inclui
                foreach ($arraydestaque as  $destaque) {
                    if( $resImportacao['sucesso'] ){
                        // verifica se já existe se sim deleta
                        $negocioImportacao->verificaExistentes( $destaque, $p["IMPA_AA_IMPORTACAO"], $this->_tipo );
                        // tabela de valores
                        $negociodestaque->incluirDados( $destaque, $resImportacao['codigo'] );
    ...
    {
        // Título da tela (action)
        $this->view->telaTitle = 'Excluir Destaque';
        ...
        if ($this->getRequest()->isGet()) {      
            ...
            if ($chavePrimaria) {
                    ...
                    $this->view->codigo = $this->_negocioDestaque->chavePrimaria();
