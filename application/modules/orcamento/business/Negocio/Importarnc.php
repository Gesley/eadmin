 ...
 */
class Orcamento_Business_Negocio_Importarnc extends Orcamento_Business_Negocio_Base {
     ...
     * @return @array
     */
    public function incluir( $nc ) {
        $matricula = $this->_sessao->retornaPerfil();

        $dados = array(
            'IMPD_CD_MATRICULA' => strtoupper($matricula['usuario']),
            'IMPD_DS_ARQUIVO_ORIGEM' => $nc["NOME_ARQUIVO"],
            'IMPD_TX_LINHA' => utf8_encode($nc['IMPD_TX_LINHA']),
            'IMPD_DH_IMPORTACAO' => new Zend_Db_Expr('SYSDATE'),
            'IMPD_DS_CLASSE_ARQUIVO' => substr($nc['NOCR_CD_NOTA_CREDITO'], 4, 2),
            'IMPD_NR_ERRO' => 0
        );
