...
class Sosti_NocController extends Zend_Controller_Action {
	/**
...
//        Zend_Debug::dump($SadTbVidcVinculacaoDoc->getSostisAssociados(547287));exit;
        /**
         * Definições
         */
        $idCaixa = Trf1_Sosti_Definicoes::CAIXA_NOC;
        $idGrupo = 4;
        $title = "CAIXA DE ENTRADA DO ESCRITÓRIO DE PROJETOS/NOC";
...
//                        $vinc[$sosti["SSOL_ID_DOCUMENTO"]][] = $caixas->getCaixaSemNivelPesq($idCaixa, $data_pesq, $order, $vincRow["VIDC_ID_DOC_VINCULADO"])->getData();
    ...
    public function baixarcaixaAction() {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if ($data['acao'] && isset($data['acao']) && $data['acao'] == 'Baixar') {
                ...
                $this->view->title = "ESCRITÓRIO DE PROJETOS/NOC - BAIXAR SOLICITAÇÃO(ES)";
                $this->view->form = $form;
            } else {
                if ($form->isValid($data)) {
                    ...
                    $this->_helper->_redirector('index', 'noc', 'sosti');
                } else {
                    $solicspace = new Zend_Session_Namespace('solicspace');
                    $this->view->data = $solicspace->dados;
                    $this->view->title = "ESCRITÓRIO DE PROJETOS/NOC - BAIXAR SOLICITAÇÃO(ÕES)";
                    $form->populate($data);
                    $this->view->form = $form;
    ...
    public function esperacaixaAction() {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if ($data['acao'] && isset($data['acao']) && $data['acao'] == 'Espera') {
                ...
                $this->view->title = "ESCRITÓRIO DE PROJETOS/NOC - COLOCAR EM ESPERA A(S) SOLICITAÇÃO(ES)";
                $this->view->form = $form;
            } else {
                if ($form->isValid($data)) {
                    $userNs = new Zend_Session_Namespace('userNs');
    ...
    public function encaminharAction() {
        ...
        /**
         * Tratamento para retirar o grupo de serviço Desenvolvimento e sustentação e gestão de infra da lista
         */
        ...
        foreach ($arr_sgrs_id_grupo as $value) {
            ...
            if (in_array($value_option["SGRS_ID_GRUPO"], array(2, 119, 120))) {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if ($data['acao'] && isset($data['acao']) && $data['acao'] == 'Encaminhar') {
                ...
                if ($form->isValid($data)) {
                    ...
                    $this->view->title = "ESCRITÓRIO DE PROJETOS/NOC - ENCAMINHAR SOLICITAÇÃO(ES)";
