...
class Sosti_AtendimentotecnicoController extends Zend_Controller_Action {
	/**
    ...
    public function segundonivelAction() {
        /**
         * Definições
         */
        $idCaixa = 1;
        ...
        $cdNivel = 2;
        $idGrupo = 1;
        $title = "2º NÍVEL - ATENDIMENTO TÉCNICO PRESENCIAL - SAP";
...
//                        $vinc[$sosti["SSOL_ID_DOCUMENTO"]][] = $caixas->getCaixaComNivelPesq($idCaixa, $cdNivel, $data_pesq, $order, $vincRow["VIDC_ID_DOC_VINCULADO"])->getData();
            ...
            foreach ($paginator->getCurrentItems() as $sosti) {
                if ($sosti['VINCULADA'] > 0) {
                    $vincs = $caixas->getVinculos($sosti["SSOL_ID_DOCUMENTO"]);
                    foreach ($vincs as $vincRow) {
                        $vinc[$sosti["SSOL_ID_DOCUMENTO"]][] = $caixas->getCaixaComNivelPesq($idCaixa, $cdNivel, null, $order, $vincRow["VIDC_ID_DOC_VINCULADO"])->getData();
                    }
                }
            }
        }
    ...
    public function baixarcaixaAction() {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if ($data['acao'] && isset($data['acao']) && $data['acao'] == 'Baixar') {
                ...
                $this->view->title = "2º Nível - Baixar solicitação(es)";
                $this->view->form = $form;
            } else {
                if ($form->isValid($data)) {
                    ...
                    $this->view->title = "2º NÍVEL - BAIXAR SOLICITAÇÃO(ÕES)";
                    $form->populate($data);
                    $this->view->form = $form;
    ...
    public function esperacaixaAction() {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if ($data['acao'] && isset($data['acao']) && $data['acao'] == 'Espera') {
                ...
                $this->view->title = "2º NÍVEL - COLOCAR EM ESPERA A(S) SOLICITAÇÃO(ES):";
                $this->view->form = $form;
            } else {
                if ($form->isValid($data)) {
                    $userNs = new Zend_Session_Namespace('userNs');
            ...
            if ($data['acao'] && isset($data['acao']) && $data['acao'] == 'Encaminhar') {
                ...
                if ($form->isValid($data)) {
                    ...
                    if ($data["ENCAMINHAMENTO"] == 'nivel') {
                        ...
                        /**
                         * Encaminhamento de solicitação para outro grupo de atendimento
                         */
                        ...
                        $this->_helper->_redirector('segundonivel', 'atendimentotecnico', 'sosti');
                        /**
                         * Encaminhamento de solicitação para caixa pessoal
                         */
                    ...
                    $this->view->title = "2º NÍVEL - ENCAMINHAR SOLICITAÇÃO(ES)";
