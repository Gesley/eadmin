...
class Sosti_SuporteespecializadoController extends Zend_Controller_Action {
    ...
    public function terceironivelAction () {
        /**
         * Definições
         */
        $idCaixa = 1;
        ...
        $cdNivel = 3;
        $idGrupo = 1;
        $title = "3º NÍVEL - ATENDIMENTO DE SUPORTE ESPECIALIZADO - SSE";
...
//                        $vinc[$sosti["SSOL_ID_DOCUMENTO"]][] = $caixas->getCaixaSemNivelPesq($idCaixa, $data_pesq, $order, $vincRow["VIDC_ID_DOC_VINCULADO"])->getData();
                        ...
                        if (is_string($queryCaixa)) {
                            ...
                            foreach ($vinc[$sosti["SSOL_ID_DOCUMENTO"]] as $key) {

                                if (!$key) {
                                    unset($vinc[$sosti["SSOL_ID_DOCUMENTO"]][$key]);
                                }
                            }
                        }
                    }
    ...
    public function baixarcaixaAction () {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if ($data['acao'] && isset($data['acao']) && $data['acao'] == 'Baixar') {
                ...
                $this->view->title = "3º NÍVEL - BAIXAR SOLICITAÇÃO(ES)";
                $this->view->form = $form;
            } else {
                if ($form->isValid($data)) {
                    ...
                    $this->_helper->_redirector('terceironivel', 'suporteespecializado', 'sosti');
                } else {
                    $solicspace = new Zend_Session_Namespace('solicspace');
                    $this->view->data = $solicspace->dados;
                    $this->view->title = "3º NÍVEL - BAIXAR SOLICITAÇÃO(ÕES)";
                    $form->populate($data);
                    $this->view->form = $form;
    ...
    public function esperacaixaAction () {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if ($data['acao'] && isset($data['acao']) && $data['acao'] == 'Espera') {
                ...
                $this->view->title = "3º NÍVEL - COLOCAR EM ESPERA A(S) SOLICITAÇÃO(ES)";
                $this->view->form = $form;
            } else {
                if ($form->isValid($data)) {
                    $userNs = new Zend_Session_Namespace('userNs');
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if ($data['acao'] && isset($data['acao']) && $data['acao'] == 'Encaminhar') {
                ...
                if ($form->isValid($data)) {
                    ...
                    $this->view->title = "3º NÍVEL - ENCAMINHAR SOLICITAÇÃO(ES)";
