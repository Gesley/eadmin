...
class Sosti_BancodadosredeController extends Zend_Controller_Action {
	/**
    ...
    public function indexAction() {
         ...
         * Definições
         */
        $idCaixa = Trf1_Sosti_Definicoes::CAIXA_INFRAESTRUTURA;
        $idGrupo = 3;
        $title = "CAIXA DE ENTRADA DA INFRAESTRUTURA";
...
//                        $vinc[$sosti["SSOL_ID_DOCUMENTO"]][] = $caixas->getCaixaSemNivelPesq($idCaixa, $data_pesq, $order, $vincRow["VIDC_ID_DOC_VINCULADO"])->getData();
    ...
    public function baixarcaixaAction() {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if ($data['acao'] && isset($data['acao']) && $data['acao'] == 'Baixar') {
                ...
                $this->view->title = "BANCO DE DADOS/ADMINISTRAÇÃO DE DADOS/REDE - BAIXAR SOLICITAÇÃO(ES)";
                $this->view->form = $form;
            } else {
                if ($form->isValid($data)) {
                    ...
                    $this->_helper->_redirector('index', 'bancodadosrede', 'sosti');
                } else {
                    $solicspace = new Zend_Session_Namespace('solicspace');
                    $this->view->data = $solicspace->dados;
                    $this->view->title = "BANCO DE DADOS/ADMINISTRAÇÃO DE DADOS/REDE - BAIXAR SOLICITAÇÃO(ÕES)";
                    $form->populate($data);
                    $this->view->form = $form;
    ...
    public function esperacaixaAction() {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if ($data['acao'] && isset($data['acao']) && $data['acao'] == 'Espera') {
                ...
                $this->view->title = "BANCO DE DADOS/ADMINISTRAÇÃO DE DADOS/REDE - COLOCAR EM ESPERA A(S) SOLICITAÇÃO(ES)";
                $this->view->form = $form;
            } else {
                if ($form->isValid($data)) {
                    $userNs = new Zend_Session_Namespace('userNs');
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if ($data['acao'] && isset($data['acao']) && $data['acao'] == 'Encaminhar') {
                ...
                if ($form->isValid($data)) {
                    ...
                    $this->view->title = "BANCO DE DADOS/ADMINISTRAÇÃO DE DADOS/REDE - ENCAMINHAR SOLICITAÇÃO(ES)";
    ...
    public function extenderprazoAction() {
        ...
        if ($this->getRequest()->isPost()) {
            $data = array_merge($this->getRequest()->getPost(), $form->populate($this->getRequest()->getPost())->getValues()); /* Aplica Filtros - Mantem Post */
            if ($data['acao'] && isset($data['acao']) && $data['acao'] == 'Estender Prazo') {
                $userNs = new Zend_Session_Namespace('userNs');
                $solicspace = new Zend_Session_Namespace('solicspace');
                $SadTbVidcVinculacaoDoc = new Application_Model_DbTable_SadTbVidcVinculacaoDoc();
                $SolicitacaoComFamilia = $SadTbVidcVinculacaoDoc->getFamiliaVinculacao($data['solicitacao']);
                ...
                $this->view->data = $data['solicitacao'];
                $id = $solicitacao_array[0];
                $this->view->title = "BANCO DE DADOS/ADMINISTRAÇÃO DE DADOS/REDE - EXTENDER O PRAZO DA(S) SOLICITAÇÃO(ES)";
                $this->view->form = $form;
            } else {
                if ($form->isValid($data)) {
                    $userNs = new Zend_Session_Namespace('userNs');
                    ...
                    foreach ($solicspace->dados as $d) {
                        ...
                        $dataHoraAnexo = $espera->prazoSolicitacao($idDocumento, $dataMofaMoviFase, $dataSspaSolicPrazoAtend);
                    }
                    $msg_to_user = "Solicitação(es) n(s)º " . substr($solicitacoesEncaminhadas, 1) . " com o prazo de atendimento extendido!";
                    $this->_helper->flashMessenger(array('message' => $msg_to_user, 'status' => 'success'));
                    $this->_helper->_redirector('index', 'bancodadosrede', 'sosti');
                } else {
                    $solicspace = new Zend_Session_Namespace('solicspace');
