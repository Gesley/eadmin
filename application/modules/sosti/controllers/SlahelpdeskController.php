...
class Sosti_SlahelpdeskController extends Zend_Controller_Action {
	/**
    ...
    public function triagemAction() {
        ...
        */
        ...
        if ($permiteSla ['permissao']) {
                ...
                $this->view->form = $form;
                $this->view->title = "SLA - HELPDESK - TRF1";
                $form->setAction('triagem');
                $Sla_Helpdesk_ns = new Zend_Session_Namespace('Sla_Helpdesk_ns');
                ...
                if ($this->getRequest()->isPost()) {
                    ...
                    if ($form->isValid($data)) {
                        ...
                        if ($timestempInicial < $timestempInicioContagem && $timestempFinal <= $timestempInicioContagem) {
                            $this->_helper->_redirector('indicadoressempedidoinformacao', 'Slahelpdesk', 'sosti');
                        } elseif ($timestempInicial > $timestempInicioContagem && $timestempFinal > $timestempInicioContagem) {
                            $this->_helper->_redirector('indicadorescompedidoinformacao', 'Slahelpdesk', 'sosti');
                            ...
                            $this->_helper->_redirector('triagem', 'Slahelpdesk', 'sosti');
                        }
                    }
                }
        }else{
    ...
    /**
     * Calcula o Indicadores de nivel de serviço sem pedido de informação
     * 
     */
    public function indicadoressempedidoinformacaoAction() {
        ...
        $possui_permissao_fechamento = $OcsTbPupePerfilUnidPessoa->getPossuiPerfilPorNome('GESTOR DO CONTRATO DO ATENDIMENTO USUÁRIO DO TRF', $userNs->matricula);
        ...
        $Sla_Helpdesk_ns = new Zend_Session_Namespace('Sla_Helpdesk_ns');
        if ($Sla_Helpdesk_ns->data != '') {
            $this->view->data = $Sla_Helpdesk_ns->data;

            $form->populate($Sla_Helpdesk_ns->data);

            $tempoTimeInterval = new App_TimeInterval();
            $fusoHorario = $tempoTimeInterval->calculaFusoHorario('DF');
            /**
             ...
             * INDICADOR IIA 1
             */
            $IIA_DADOS = $indicadorNivelServ->getIndicadorPorCaixaeSiglaIndicador(1, 'IIA');
            $ISS_DADOS = $indicadorNivelServ->getIndicadorPorCaixaeSiglaIndicador(1, 'ISS');
            $IAP_DADOS = $indicadorNivelServ->getIndicadorPorCaixaeSiglaIndicador(1, 'IAP');
            $ISD_DADOS = $indicadorNivelServ->getIndicadorPorCaixaeSiglaIndicador(1, 'ISD');
            $ITP_DADOS = $indicadorNivelServ->getIndicadorPorCaixaeSiglaIndicador(1, 'ITP');
            $INC_DADOS = $indicadorNivelServ->getIndicadorPorCaixaeSiglaIndicador(1, 'INC');
            $ICR_DADOS = $indicadorNivelServ->getIndicadorPorCaixaeSiglaIndicador(1, 'ICR');

            $solicitacoesIia = $indicadorNivelServ->getAtendUsuDatasSLA_IIA(1, 1, $Sla_Helpdesk_ns->data['DATA_INICIAL'], $Sla_Helpdesk_ns->data['DATA_FINAL'], $fusoHorario, $IIA_DADOS['SINS_ID_INDICADOR']);
...
//            Zend_Debug::dump(count($solicitacoesIia),'$solicitacoesIia count');
             ...
             * INDICADOR ISS 2
             */
            $solicitacoesIss = $indicadorNivelServ->getAtendUsuDatasSLA_ISS(1, $Sla_Helpdesk_ns->data['DATA_INICIAL'], $Sla_Helpdesk_ns->data['DATA_FINAL'], $fusoHorario, $ISS_DADOS['SINS_ID_INDICADOR'], $IAP_DADOS['SINS_ID_INDICADOR'], $ISD_DADOS['SINS_ID_INDICADOR']);
...
//            Zend_Debug::dump(count($solicitacoesIsd),'$solicitacoesIsd count');
             ...
             * INDICADOR ITP 4
             */
//            $solicitacoesItp = $indicadorNivelServ->getAtendUsuDatasSLA_ITP(1, $Sla_Helpdesk_ns->data['DATA_INICIAL'], $Sla_Helpdesk_ns->data['DATA_FINAL']);
...
//            $solicitacoesIap = $solicitacoesIss;
             ...
             * INDICADOR INC 6
             */
            $solicitacoesInc = $indicadorNivelServ->getAtendUsuDatasSLA_INC(1, $Sla_Helpdesk_ns->data['DATA_INICIAL'], $Sla_Helpdesk_ns->data['DATA_FINAL'], $fusoHorario, $INC_DADOS['SINS_ID_INDICADOR']);
//            Zend_Debug::dump(count($solicitacoesInc),'$solicitacoesIss count');

            /**
             * INDICADOR ICR 7 
             */
            $solicitacoesIcr = $indicadorNivelServ->getDatasSLA_ICR(1, $Sla_Helpdesk_ns->data['DATA_INICIAL'], $Sla_Helpdesk_ns->data['DATA_FINAL'], $fusoHorario, $ICR_DADOS['SINS_ID_INDICADOR']);
//            Zend_Debug::dump(count($solicitacoesInc),'$solicitacoesIss count');
             ...
             * Índice de Início de Atendimento no Prazo
             */
...
//            Zend_Debug::dump($RecusadasIcrSolicitacoes,'$RecusadasIcrSolicitacoes');
            ...
            $meta[3] = 'em desenvolvimento'; // ITP – Índice de Ligações Telefônicas Perdidas
...
//            $meta[7] = $totalDpig.'%';//Deixar de Prestar as Informações Gerenciais
            ...
            $glosa[3] = 'em desenvolvimento'; // ITP – Índice de Ligações Telefônicas Perdidas
...
//            $glosa[7] = $glosaDpig.'%'; //Deixar de Prestar as Informações Gerenciais
            ...
            $indMin->title = "SLA - HELPDESK - TRF1";
            $indMin->periodo = 'PERÍODO: ' . $Sla_Helpdesk_ns->data['DATA_INICIAL'] . ' À ' . $Sla_Helpdesk_ns->data['DATA_FINAL'];
            $indMin->fuso = "";
...
//            Zend_Debug::dump(count($solicitacoesInc),'$solicitacoesIss count');
            ...
            $expedienteNormal = $SosTbGrexGrupoServExped->getExpedientePorGrupoPorNomeExpediente(2, "SLA");
...
//            }
//            
			$regraSla = new Trf1_Sosti_Negocio_Sla();
			$dtIni = substr($Sla_Helpdesk_ns->data['DATA_INICIAL'], 6, 4) . '-' . substr($Sla_Helpdesk_ns->data['DATA_INICIAL'], 3, 2) . '-' . substr($Sla_Helpdesk_ns->data['DATA_INICIAL'], 0, 2);
			$dtFim = substr($Sla_Helpdesk_ns->data['DATA_FINAL'], 6, 4) . '-' . substr($Sla_Helpdesk_ns->data['DATA_FINAL'], 3, 2) . '-' . substr($Sla_Helpdesk_ns->data['DATA_FINAL'], 0, 2);
			
			$solicitacoesItp = $regraSla->retornaPercentualLigacoesPerdidas($dtIni, $dtFim);
			$totalItp = (float) sprintf('%3.2f', $solicitacoesItp);
			
			// Define glosa, se houver
			$glosaItp = '0%';
			
			if (($totalItp >= 10.01) && ($totalItp <= 11)) {
				$glosaItp = '2%';
			}
			if (($totalItp >= 11.01) && ($totalItp <= 12)) {
				$glosaItp = '4%';
			}
			if (($totalItp >= 12.01) && ($totalItp <= 13)) {
				$glosaItp = '7%';
			}
			if ($totalItp > 13) {
				$glosaItp = 'Não previsto em contrato!';
			}
			
			/*
			Zend_Debug::dump($dtIni);
			Zend_Debug::dump($dtFim);
			
			Zend_Debug::dump($Sla_Helpdesk_ns->data['DATA_INICIAL']);
			Zend_Debug::dump($Sla_Helpdesk_ns->data['DATA_FINAL']);
			
			Zend_Debug::dump($solicitacoesItp);
			Zend_Debug::dump($totalItp);
			exit;
			*/
...
//            Zend_Debug::dump($RecusadasIcrSolicitacoes,'$RecusadasIcrSolicitacoes');
            ...
            $meta[1] = $totalIss . '%'; // ISS – Índice de Soluções das Solicitações no Prazo
            $meta[2] = $totalIsd . '%'; // ISD – Índice de Chamados Solucionados no mesmo dia
            $meta[3] = $totalItp . '%'; // ITP – Índice de Ligações Telefônicas Perdidas
            $meta[4] = $totalIap . '%'; // IAP – Índice de Ausência de Prazo
            $meta[5] = $totalInc . '%';  // INC – Índice de chamados com Não Conformidade
            $meta[6] = $totalIcr . '%'; // ICR – Índice de chamados reabertos
//            $meta[7] = $totalDpig.'%';//Deixar de Prestar as Informações Gerenciais
            ...
            $glosa[1] = $glosaIss . '%'; // ISS – Índice de Soluções das Solicitações no Prazo
            $glosa[2] = $glosaIsd . '%'; // ISD – Índice de Chamados Solucionados no mesmo dia
            $glosa[3] = $glosaItp; // ITP – Índice de Ligações Telefônicas Perdidas
            $glosa[4] = $glosaIap . '%';  // IAP – Índice de Ausência de Prazo
            $glosa[5] = $glosaInc . '%';  // INC – Índice de chamados com Não Conformidade
            $glosa[6] = $glosaIcr . '%'; // ICR – Índice de chamados reabertos
//            $glosa[7] = $glosaDpig.'%'; //Deixar de Prestar as Informações Gerenciais
            ...
            if($param == 'pdf'){
                ...
                $response->clearBody();
                $body = str_replace(substr($body, strpos($body, '<script>'), strpos($body, '</script>')+9), '', $body);
                $body = str_replace(substr($body, strpos($body, '<script>'), strpos($body, '</script>')+9), '', $body);
                $body = str_replace(substr($body, strpos($body, '<script>'), strpos($body, '</script>')+9), '', $body);
