...
class Sosti_DesenvolvimentosustentacaoController extends Zend_Controller_Action {
    /**
    ...
    $userNs = new Zend_Session_Namespace('userNs');
        ...
        foreach ($arr_fadm_id_fase as $value) {
            if (!(($value == 1019) || ($value == 1006) || ($value == 1022) || ($value == 1013) || ($value == 1001) || ($value == 1007) || ($value == 1025) ||
                    ($value == 1024) || ($value == 1008) || ($value == 1092) )
            ) {
                if ($value != "") {
...
//            Zend_Debug::dump($caixas->getVinculos(8054));exit;
    ...
    public function baixarcaixaAction() {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if ($data['acao'] && isset($data['acao']) && $data['acao'] == 'Baixar') {
                ...
                $verficaVinculacao = new Application_Model_DbTable_SadTbVidcVinculacaoDoc();
                $familia = $verficaVinculacao->getFamiliaVinculacao($data['solicitacao']);
                if ($familia) {
                    $data['solicitacao'] = $familia;
                }
                $solicspace->dados = $data['solicitacao'];
                $this->view->data = $data['solicitacao'];
                $this->view->title = "DESENVOLVIMENTO E SUSTENTAÇÃO - PREPOSTO/GESTOR - BAIXAR SOLICITAÇÃO(ES)";
                $this->view->form = $form;
            } else {
                if ($form->isValid($data)) {
                    ...
                    $this->_helper->_redirector('index', 'desenvolvimentosustentacao', 'sosti');
                } else {
                    $solicspace = new Zend_Session_Namespace('solicspace');
                    $this->view->data = $solicspace->dados;
                    $this->view->title = "DESENVOLVIMENTO E SUSTENTAÇÃO - PREPOSTO/GESTOR - BAIXAR SOLICITAÇÃO(ÕES)";
                    $form->populate($data);
                    $this->view->form = $form;
    ...
    public function homologarcaixaAction() {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if ($data['acao'] && isset($data['acao']) && $data['acao'] == 'Homologar') {
                ...
                $solicspace->dados = $data['solicitacao'];
                $this->view->data = $data['solicitacao'];
                $this->view->title = "DESENVOLVIMENTO E SUSTENTAÇÃO - PREPOSTO/GESTOR - HOMOLOGAR SOLICITAÇÃO(ES)";
                $this->view->form = $form;
            } else {
                if ($form->isValid($data)) {
                    ...
                    foreach ($solicspace->dados as $d) {
                        ...
                        $corpo = "Uma solicitação foi colocada em homologação, será necessário acessar o sistema</p>
                                    Número da Solicitação: " . $dados_input['DOCM_NR_DOCUMENTO'] . " <br/>
                                    Data da Solicitação: " . $dados_input["DATA_ATUAL"] . " <br/>
                                    Atendente: " . $userNs->nome . " <br/>
                                    Tipo de Serviço Solicitado: " . $dados_input['SSER_DS_SERVICO'] . "<br/>
                    ...
                    $msg_to_user = "Solicitação(es) n(s)º " . substr($solicitacoesEncaminhadas, 1) . " colocada em homologação!";
                    $this->_helper->flashMessenger(array('message' => $msg_to_user, 'status' => 'success'));
                    $this->_helper->_redirector('index', 'desenvolvimentosustentacao', 'sosti');
                } else {
                    $solicspace = new Zend_Session_Namespace('solicspace');
                    $this->view->data = $solicspace->dados;
                    $this->view->title = "DESENVOLVIMENTO E SUSTENTAÇÃO - PREPOSTO/GESTOR - HOMOLOGAR SOLICITAÇÃO";
                    $form->populate($data);
                    $this->view->form = $form;
    ...
    public function esperacaixaAction() {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if ($data['acao'] && isset($data['acao']) && $data['acao'] == 'Espera') {
                ...
                $this->view->title = "DESENVOLVIMENTO E SUSTENTAÇÃO PREPOSTO/GESTOR - COLOCAR EM ESPERA A(S) SOLICITAÇÃO(ES)";
                $this->view->form = $form;
            } else {
                if ($form->isValid($data)) {
                    $userNs = new Zend_Session_Namespace('userNs');
    ...
    public function encaminharAction() {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if ($data['acao'] && isset($data['acao']) && $data['acao'] == 'Encaminhar') {
                ...
                $form->addElement(new Zend_Form_Element_Hidden('LABEL_ACOMPANHAR_BAIXA'));
                if ($ultimaPosicao["FADM_ID_FASE"] == 1092) {
                    $acaoTitulo = 'ENCAMINHAR ORDEM(NS) DE SERVIÇO(S)';
                    $this->view->labelTabela = 'N. da ordem de serviço';
                    $form->LABEL_ACOMPANHAR_BAIXA->setValue('Quero Acompanhar a baixa desta Ordem de Serviço');
                } else {
                    $acaoTitulo = 'ENCAMINHAR SOLICITAÇÃO(ES)';
                    $this->view->labelTabela = 'N. da solicitação';
                    $form->LABEL_ACOMPANHAR_BAIXA->setValue('Quero Acompanhar a baixa desta Solicitação');

                }
                $this->view->title = "DESENVOLVIMENTO E SUSTENTAÇÃO - PREPOSTO/GESOTOR - ".$acaoTitulo;
                $verficaVinculacao = new Application_Model_DbTable_SadTbVidcVinculacaoDoc();
                $familia = $verficaVinculacao->getFamiliaVinculacao($data['solicitacao']);
                if ($familia) {
                    $data['solicitacao'] = $familia;
                ...
                if ($form->isValid($data)) {
                    ...
                    if ($data["ENCAMINHAMENTO"] == 'nivel') {
                        ...
                        foreach ($dataSol["solicitacao"] as $d) {
                            ...
                            $dataMofaMoviFase["MOFA_ID_MOVIMENTACAO"] = $movimentacao;
                            $dataMofaMoviFase["MOFA_CD_MATRICULA"] = $userNs->matricula;
                            $dataMofaMoviFase["MOFA_CD_MATR_ENCAMINHADO"] = $data['APSP_ID_PESSOA'];
                            $dataMofaMoviFase["MOFA_DS_COMPLEMENTO"] = $data['MOFA_DS_COMPLEMENTO'];
