<?php
class Sisad_Form_Formularioinformacao extends Zend_Form {
   ...
   public function init()
   {
        ...
        $docm_nr_dcmto_usuario->setRequired(false)
                          ->addFilter('Alnum')
                          ->setLabel('Número Informação do Usuário:')
        ...
        $docm_cd_referencia = new Zend_Form_Element_Text('DOCM_CD_REFERENCIA');
        $docm_cd_referencia ->setRequired(true)
                            ->setLabel('*Referência Documento')
                            ->addFilter('StripTags')
                            ->setOptions(array('style' => 'width:500px'))
                            ->addValidator('NotEmpty');
