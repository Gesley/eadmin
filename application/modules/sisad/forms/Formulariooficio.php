<?php
class Sisad_Form_Formulariooficio extends Zend_Form {
   ...
   public function init()
   {
        ...
        $docm_nr_dcmto_usuario->setRequired(false)
                          ->addFilter('Alnum')
                          ->setLabel('Número Ofício do Usuário:')
                          ->setOptions(array('maxLength' => 24));

        $docm_flag_nr_processo = new Zend_Form_Element_Radio('DOCM_FLAG_NR_PROCESSO');
        $docm_flag_nr_processo ->setRequired(true)
                               ->setLabel('*Possui Processo:')
                               ->setValue('N')
                               ->setMultiOptions(array('S'=>'Sim', 'N'=>'Não'));

        $docm_nr_processo = new Zend_Form_Element_Text('DOCM_NR_PROCESSO');
        $docm_nr_processo ->setRequired(false)
                          ->addFilter('Alnum')
