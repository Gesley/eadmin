<?php
class Sisad_FormulariooficioController extends Zend_Controller_Action
{
	/**
    ...
    {
        $this->view->title = "Formulário de Ofício";
    }

   public function formAction()
    {
        $form   = new Sisad_Form_Formulariooficio();
//        $table  = new Sisad_Model_DataMapper_Documento();
    ...
    public function ajaxnomematriculasolicitanteAction()
    {
        $matriculanome     = $this->_getParam('term','');
        $OcsTbPmatMatricula = new Application_Model_DbTable_OcsTbPmatMatricula();
        $nome_array = $OcsTbPmatMatricula->getNomeMatriculaSolicitanteAjax($matriculanome);
    ...
    public function visualizarAction() {
        $this->view->title = "Pré-visualização do Ofício";
        $form = new Sisad_Form_Formulariooficio();

        $userNamespace = new Zend_Session_Namespace('userNs');
        $PLocalidade = new Application_Model_DbTable_PLocalidade();
        $mapperPctt = new Arquivo_Model_DataMapper_Pctt();
        $parte = new Application_Model_DbTable_Parte();
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if ($form->isValid($data)) {
                ...
                if ($data["DOCM_NR_PROCESSO"]){
                    $partes = $parte->getPartesProcesso($data["DOCM_NR_PROCESSO"]);
                }
                $uf = substr($userNamespace ->matricula,0,2);
                    if ($uf == 'TR'){
                        $uf = 'DF';
                    }
                $this->view->cidade = $PLocalidade->getLocalByUF($uf);
                ...
                $this->view->siglaunidadeemissora = $docm_cd_lotacao_geradora_array[0];

                if ($data["DOCM_NR_PROCESSO"]){
                    $this->render('visualizarprocesso');
                }else{
                    $this->render();
                }
                ...
                $name =  'SISAD_TEMP_DOC_OFICIO_VISUALIZAR_DOCUMENTO' . date("dmYHisu") .'.pdf';
                            ...
                            try {
                                ...
                                $rowDocmDocumento = $tabelaSadTbDocmDocumento->fetchRow("DOCM_ID_DOCUMENTO = $idDocumento");
                                 $userNs          = new Zend_Session_Namespace('userNs');
                                    ...
                                    $fullUrl      = $this->getFrontController()->getBaseUrl();  
                                    $corpo        = "Cadastro efetuado com sucesso</p>
                                              Número do Documento: <a href=\"http://$http/$fullUrl/sisad/pesquisadcmto/pesquisa-doc-email/DOCM_ID_DOCUMENTO/" . $rowDocmDocumento['DOCM_NR_DOCUMENTO'] . "\"><b>" . $rowDocmDocumento['DOCM_NR_DOCUMENTO'] . "</b> </a><br/>";
...
//                                $this->_helper->flashMessenger ( array('message' => $msg_to_user, 'status' => 'error'));
                            ...
                            $this->_helper->_redirector('form','Formulariooficio','sisad');
                        } else {
                            /*tratamento de erro*/
