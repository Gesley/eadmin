<?php
class Sisad_Model_DataMapper_Documento
{
    ...
    public function getNumeroSequencialDCMTO($codSecaoReda,$codDocUnidReda,$codDocTipoDoc)
    {
        ...
        $stmt = $this->db->query("SELECT  NVL(MAX(DOCM_NR_SEQUENCIAL_DOC)+1,1) AS DOCM_NR_SEQUENCIAL_DOC
    ...
    public function getCodSecSubsec_PaiSecao($sgSecao,$codLotacao)
    {
        ...
        $stmt = $this->db->query(" SELECT SESB_SESU_CD_SECSUBSEC /*CODIGO SEÇÃO DA LOTAÇÃO PAI SEÇAÕ OU TRIBUNAL*/
    ...
    public function getUltimoTelefoneCadastrado($matricula)
    {
        ...
        $stmt = $this->db->query("SELECT A.SSOL_DS_EMAIL_EXTERNO,A.SSOL_NR_TELEFONE_EXTERNO
...
//            exit;
            ...
            $rowDocmDocumento_cancelar = $tabelaSadTbDocmDocumento->find($dataMofaMoviFase_aux["DOCM_ID_DOCUMENTO"])->current();;
...
//            exit;
    ...
    public function getDadosProcesso($idProcesso)
    {
        ...
        $stmt = $this->db->query("SELECT DTPD.DTPD_NO_TIPO,
                                   DOCM.DOCM_ID_DOCUMENTO,
                                   DOCM.DOCM_NR_DOCUMENTO,
                                   DOCM.DOCM_NR_DCMTO_USUARIO,
    ...
    public function getDadosDocumentoByNrDoc($numeroDoc)
    {
        ...
        $stmt = $this->db->query("SELECT DOCM_ID_DOCUMENTO FROM SAD_TB_DOCM_DOCUMENTO WHERE DOCM_NR_DOCUMENTO_RED = $numeroDoc");
        return $stmt->fetchAll();
    }
  public function getDocumentoIdByNrDoc($numeroDoc)
    {
        
        $stmt = $this->db->query("SELECT DOCM_ID_DOCUMENTO FROM SAD_TB_DOCM_DOCUMENTO WHERE DOCM_NR_DOCUMENTO = $numeroDoc");
        return $stmt->fetchAll();
    }
...
//        Zend_Debug::dump($q);exit;
    ...
    public function getPesquisaDocEmail($id){
        
        $stm = $this->db->query("SELECT DISTINCT DOCM.DOCM_ID_DOCUMENTO,
                                       DTPD.DTPD_NO_TIPO,
                                       DOCM.DOCM_NR_DOCUMENTO,
                                       DOCM.DOCM_NR_DCMTO_USUARIO,
                                       DOCM_DH_CADASTRO,
...
public function getDocumentosUnidade($siglasecao,$codlotacao, $matricula, $order, $parametro){
    ...
    public function getIdProcesso($idDocumento){
        ...
        $stmt = $this->db->query("SELECT DCPR.DCPR_ID_PROCESSO_DIGITAL, 
...
//           Zend_Debug::dump($q);exit;
    ...
    public function getExtensoesAceitas()
    {
        $db = Zend_Db_Table_Abstract::getDefaultAdapter();
        $q = "SELECT TPEX_ID_TP_EXTENSAO, TPEX_DS_TP_EXTENSAO
              FROM SAD_TB_TPEX_TIPO_EXTENSAO
              WHERE TPEX_DS_TP_EXTENSAO NOT IN ('exe')";
        $stmt = $db->query($q);
        $array =  $stmt->fetchAll();
        $arrExtensao = array();
        $i = 0;
        foreach ($array as $arr) {
            $arrExtensao[$i] = $arr['TPEX_DS_TP_EXTENSAO'];
            $i++;
        }
        return $arrExtensao;
    }
